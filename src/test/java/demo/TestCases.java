...
public class TestCases extends ExcelDataProvider { // Lets us read the data
        ...
        public void testCase01() throws InterruptedException {
                try {
                        ...
                        Assert.assertEquals(currentpageUrl, "https://www.youtube.com/",
                                        "Alert! You are not on Youtube page!");
        ...
        public void testCase02() throws InterruptedException {
                ...
                Thread.sleep(3000);
                String certificationtext = Wrappers.extractElementText(driver,
                                By.xpath("(//p[@class='style-scope ytd-badge-supported-renderer'])[last()]"));
                System.out.println("Movie Certificate Text: " + certificationtext);
                SoftAssert sassert = new SoftAssert();
                sassert.assertEquals(certificationtext, "U");
                String genretextfull = Wrappers.extractElementText(driver, By.xpath(
                                "(//span[@class='grid-movie-renderer-metadata style-scope ytd-grid-movie-renderer'])[last()]"));
        ...
        //         // Wrappers.clickOnRightScrollOnElementUntillLast(driver, By.xpath("(//button[@aria-label='Next'])[1]"));
        //         Wrappers.clickOnRightScrollOnElementUntillLast(driver, By.xpath("(//button[@aria-label='Next' and @aria-disabled='false'])[1]"));

                

        //         Thread.sleep(4000);
        //         String noofvideostext = Wrappers.extractElementText(driver,
        ...
        public void testCase03() {
                ...
                wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@title='Music']"))); // Wait for the 'Music' link to be visible

                // Click on the Music tab
                driver.findElement(By.xpath("//a[@title='Music']")).click();
                wait.until(ExpectedConditions.urlContains("music")); // Wait until the Music page loads

                // Scroll to the right using the Next button
                WebElement nextButton = driver.findElement(By.xpath("(//button[@aria-label='Next' and @aria-disabled='false'])[1]"));
                while (nextButton.isEnabled()) {
                        nextButton.click();
                        wait.until(ExpectedConditions.elementToBeClickable(nextButton)); // Wait for the button to be clickable again
                }
                ...
                // Extract the video count text
                String noofvideostext = driver.findElement(By.xpath("(//p[@id='video-count-text'])[11]")).getText();
                System.out.println("Text of video count: " + noofvideostext);
                
                // Parse the video count number
                String intpart = noofvideostext.split(" ")[0]; // Splitting the text to get the first part, which should be the number
                System.out.println("Split video count: " + intpart);
                int videocount = Integer.parseInt(intpart);
                System.out.println("Video Count: " + videocount);

                // Soft Assert to check if the video count is <= 50
                SoftAssert s1 = new SoftAssert();
                s1.assertTrue(videocount <= 50, "Video count exceeds 50");
                s1.assertAll(); // Perform the soft assertions
                } catch (Exception e) {
                System.out.println("Test Failed: " + e.getMessage());
                }
        }
        ...
        public void testCase04() throws InterruptedException {
                ...
                Thread.sleep(3000);
                // WebElement
                // latestnewselem=driver.findElement(By.xpath("(//div[@id='contents'])[18]"));
                List<WebElement> lnews = driver.findElements(
                                By.xpath("//div[@class='style-scope ytd-post-renderer' and @id='dismissible']"));
                int count = 0;
                int likescount = 0;
                for (WebElement elem : lnews) {
                        ...
                        String newstitle = elem.findElement(By.xpath(".//span[@class='style-scope ytd-post-renderer']"))
                                        .getText();
        ...
        public void testCase05(String searchTerm) throws InterruptedException {
                ...
                while (totalViews < 100_000_000) { // Stop when total views exceed 10 crore (100 million)
                        ...
                        List<WebElement> videoElements = driver.findElements(
                                        By.xpath("//span[contains(@class,'style-scope ytd-video-meta-block')]"));
                ...
                s1.assertTrue(totalViews >= 100_000_000,
                                "Total views for '" + searchTerm + "' did not reach 10 crore!");
